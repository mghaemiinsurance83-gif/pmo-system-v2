...
import {
  ...
  Database, FileSpreadsheet, Handshake, UsersRound, Moon, Sun, Menu, X, CalendarClock, LogIn, Shield,
} from "lucide-react";
...
import { LoginView } from "@/components/pmo/auth/login-view";
import { PortalApp } from "@/components/pmo/portal/portal-app";
import { AdminApp } from "@/components/pmo/admin/admin-app";
import { apiFetch } from "@/lib/api";

type ViewId =
  | "dashboard" | "tree" | "gantt" | "programs" | "participation" | "collaboration"
  | "dictionary" | "reports" | "datamodel" | "rawdata";
...
function ThemeToggle() {
  ...
  return (
    <Button variant="ghost" size="icon" suppressHydrationWarning onClick={() => setTheme(theme === "dark" ? "light" : "dark")} title={theme === "dark" ? "حالت روشن" : "حالت تاریک"}>
...
export default function Home() {
  ...
  if (mode === "login" && !isAuthed) {
    return <LoginView onSuccess={() => setMode("portal")} onCancel={() => setMode("public")} />;
  }
  ...
  const changeView = (v: ViewId) => { setView(v); setMobileOpen(false); };
    ...
    <div className="flex min-h-screen flex-col bg-background">
      <header className="sticky top-0 z-40 border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/80">
        <div className="flex h-14 items-center gap-3 px-4 sm:px-6">
          ...
          <div className="mr-auto flex items-center gap-1.5">
            {refLabel && (
              <span className="hidden sm:inline-flex items-center gap-1.5 rounded-full border border-teal-200/60 bg-teal-50 dark:border-teal-900/40 dark:bg-teal-950/30 px-2.5 py-1 text-[11px] font-medium text-teal-700 dark:text-teal-300" title="تاریخ امروز سیستم">
                <CalendarClock className="h-3.5 w-3.5" />
                امروز: {refLabel}
              </span>
            )}
            <Button variant="outline" size="sm" onClick={() => setMode(isAuthed ? "portal" : "login")} className="gap-1.5">
              <LogIn className="h-4 w-4" />
              {isAuthed ? "ورود به پورتال" : "ورود به پورتال"}
            </Button>
            <ThemeToggle />
          </div>
        </div>
        ...
        <aside className={cn("fixed lg:sticky top-14 z-30 h-[calc(100vh-3.5rem)] w-64 shrink-0 border-l bg-sidebar transition-transform lg:translate-x-0", mobileOpen ? "translate-x-0" : "translate-x-full lg:translate-x-0")}>
          <nav className="flex h-full flex-col gap-1 overflow-y-auto custom-scroll p-3">
            ...
            <div className="mt-auto rounded-lg border bg-card/50 p-3 text-[11px] text-muted-foreground">
              ...
              <p className="leading-relaxed">سامانه یکپارچه مدیریت پروژه‌ها و گام‌های کاری با احراز هویت سازمانی و پورتال اختصاصی واحدها.</p>
              <div className="mt-2 pt-2 border-t">
                <p className="flex items-center gap-1 text-teal-700 dark:text-teal-300">
      ...
      <footer className="mt-auto border-t bg-background">
        <div className="mx-auto max-w-[1600px] px-4 py-3 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-2 text-[11px] text-muted-foreground">
          ...
          <p className="flex items-center gap-1.5">
            <span>احراز هویت + پورتال واحدها</span>
            <span className="opacity-50">•</span>
