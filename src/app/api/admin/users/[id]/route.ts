...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (body.liaisonOrgIds !== undefined) {
    await db.userLiaisonOrg.deleteMany({ where: { userId: params.id } });
    if (body.liaisonOrgIds.length) {
      await db.userLiaisonOrg.createMany({
        data: body.liaisonOrgIds.map((orgId) => ({ userId: params.id, orgId, assignedBy: currentUser.id })),
      });
    }
  }

  invalidateScope(params.id);
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!currentUser) return Response.json({ error: { code: "UNAUTHORIZED" } }, { status: 401 });
  if (currentUser.role !== "ADMIN") return Response.json({ error: { code: "FORBIDDEN" } }, { status: 403 });
  if (currentUser.id === params.id) return Response.json({ error: { code: "FORBIDDEN", message: "نمی‌توانید خود را حذف کنید" } }, { status: 403 });
